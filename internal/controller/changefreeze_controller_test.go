...
var _ = Describe("ChangeFreeze Controller", func() {
	Context("When reconciling a resource", func() {
		...
		changefreeze := &freezeoperatorv1alpha1.ChangeFreeze{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind ChangeFreeze")
			err := k8sClient.Get(ctx, typeNamespacedName, changefreeze)
			if err != nil && errors.IsNotFound(err) {
				now := time.Now().UTC()
				resource := &freezeoperatorv1alpha1.ChangeFreeze{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ChangeFreeze")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ChangeFreezeReconciler{
				Client: k8sClient,
